...
from pivalve import PIValve

from app import run

l1 = [1,2,3,4,6,21,7,8,9,20,10,11] # clockwise
...
def main(loops):  
    ...
    for n_epi in range(total_episodes):
        ...
        if n_epi%print_interval==0 and n_epi!=0:
            progress = int(n_epi/total_episodes*100)
            print("# of episode :{}, avg score : {:.1f}".format(n_epi, score/print_interval))
            score = 0.0
    #env.close()
    no_valves, total = env.get_no_valves()
    return no_valves, total, env.valves, env.n
